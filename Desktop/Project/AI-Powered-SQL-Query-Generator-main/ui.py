...
if st.button("⚡ Generate SQL"):
    response = requests.post(f"{API_URL}/generate_sql/", json={"query": natural_language_query})
    if response.status_code == 200:
...
if st.button("🚀 Run Query"):
    response = requests.post(f"{API_URL}/execute_sql/", json={"query": manual_sql_query})
    if response.status_code == 200:
